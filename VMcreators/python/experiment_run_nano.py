...
flavorid = get_flavor_id (nclient,'m1.nano')
imageid = get_images_constant('cirros')
reqCurrTime = str(datetime.now())
logger.info("Create " + str(vmreqNum) + "servers with nano flavor at : " + reqCurrTime)
...
logger.info("waiting for 400 seconds " )
time.sleep(400)
...
for vms in server_list :
    ...
    if "ACTIVE" in vmstatus :
        ...
        logger.info("Deleteing servers with nano flavor and exiting")
        delete_servers(server_list)
        time.sleep(60)
        sys.exit(0)

    vmdetails.update({'name' : vmname ,'host' : vmhost, 'VMST' : vmst, 'reqTime' : reqCurrTime, 'flavor' : 'nano', 'noVMreq' : vmreqNum})
    vmlist.append(vmdetails)

logger.info("Deleteing " + str(vmreqNum) + " servers with nano flavor")
# Delete all the created servers
delete_servers(server_list)
logger.info("Collected data for nano VMs")
logger.info(vmlist)
timestr = time.strftime("%Y%m%d-%H%M%S")
...
logger.info("Run Completed for nano")
