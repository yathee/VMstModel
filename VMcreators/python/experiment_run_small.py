...
#######################################################################################################################
vmreqNum = random.randint(1,10)
vmlist = []
flavorid = get_flavor_id (nclient,'m1.small')
imageid = get_images_constant('Debian')
server_list = []

reqCurrTime = str(datetime.now())
logger.info("Create " + str(vmreqNum) + "servers with small flavor at : " + reqCurrTime)
...
for vms in server_list :
    ...
    if "ACTIVE" in vmstatus :
        ...
        logger.info("Deleteing servers with small flavor and exiting")
    ...
    vmdetails.update({'name' : vmname ,'host' : vmhost, 'VMST' : vmst, 'reqTime' : reqCurrTime,  'flavor' : 'small', 'noVMreq' : vmreqNum})
    vmlist.append(vmdetails)

logger.info("Deleteing " + str(vmreqNum) + " servers with Small flavor")
# Delete all the created servers
delete_servers(server_list)
logger.info("Collected data for Small VMs")
logger.info(vmlist)
timestr = time.strftime("%Y%m%d-%H%M%S")
write_to_file(vmlist)

logger.info("waiting for 60 seconds for completion of deleteing servers  " )
time.sleep(60)



logger.info("Run Completed for small")
