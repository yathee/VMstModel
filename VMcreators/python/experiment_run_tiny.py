...
# | 5e4833c6-b177-4c6a-9b83-cef8aba48ca1 | OpenSuse | active |
# +--------------------------------------+----------+--------+
    ...
    elif imname == 'OpenSuse' :
        return '5e4833c6-b177-4c6a-9b83-cef8aba48ca1'
...
vmreqNum = random.randint(1,15)
vmlist = []

flavorid = get_flavor_id (nclient,'m1.tiny')
imageid = get_images_constant('Ubuntu')
reqCurrTime = str(datetime.now())
logger.info("Create " + str(vmreqNum) + "servers with Tiny flavor at : " + reqCurrTime)
...
for vms in server_list :
    ...
    if "ACTIVE" in vmstatus :
        try:
            ...
            lines = textobj.split('\n')
            for l in lines :
                if l.find('Cloud-init v. 18.2 finished') != -1 :
                    dataline=l.split(" ")
        ...
        logger.info("Deleteing servers with Tiny flavor and exiting")
    ...
    vmdetails.update({'name' : vmname ,'host' : vmhost, 'VMST' : vmst, 'reqTime' : reqCurrTime, 'flavor' : 'tiny', 'noVMreq' : vmreqNum})
    vmlist.append(vmdetails)

logger.info("Deleteing " + str(vmreqNum) + " servers with Tiny flavor")
# Delete all the created servers
delete_servers(server_list)
logger.info("Collected data for Tiny VMs")
logger.info(vmlist)
timestr = time.strftime("%Y%m%d-%H%M%S")
write_to_file(vmlist)
logger.info("waiting for 30 seconds for completion of deleteing servers  " )
time.sleep(60)

logger.info("Run Completed for Tiny")
